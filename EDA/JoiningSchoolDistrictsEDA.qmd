...
best_sim_census_counties <- get_best_similarities(
...
test_mth_all_admindist_gys_counties <- test_mth_all_admindist_gys |>
  left_join(best_sim_census_counties, by = c('sedaadmin' = 'sedaadmin')) |>
  select(
    sedaadmin, seda_district.x, stateabb.x, `County Names`, gys_mn_2022_ol.x
  ) |>
  rename(
    stateabb = stateabb.x
  )

### Code for testing the best similarity function with a dataset from last semester
#best_sim_acs_income <- get_best_similarities(test_mth_all_admindist_gys, acs_income_2021_cleaned, 'seda_district', 'school_district', 0.3, 6)


### Everything gets perfectly matched, jaccard diff and string diff 0
best_sim_scp_data <- get_best_similarities(
  test_mth_all_admindist_gys_counties,
  trim_scp_county_data, 
  'County Names', 
  'County', 
  0.9, 10
  )

comp <- comprehensive_dataset |>
  left_join(best_sim_scp_data, by = c('sedaadmin' = 'sedaadmin')) |>
  select(
    sedaadmin, seda_district.x.x, poverty_percent, occupancy_percentage, 
    total_revenue, total_fed_revenue, gys_mn_2022_ol.x.x, 
    LearningModel, EnrollmentTotal, EnrollmentInPerson, EnrollmentHybrid, 
    EnrollmentVirtual, `County-Level Index`, `Family Unity`, `Community Health`,
    `Institutional Health`, `Collective Efficacy`, `County`
  ) |>
  rename(
    seda_district = seda_district.x.x,
    gys_mn_2022_ol = gys_mn_2022_ol.x.x
  )
```
