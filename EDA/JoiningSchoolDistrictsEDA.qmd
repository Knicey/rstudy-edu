...
trim_acs_bachelor <- trim_acs_dataset(acs_bachelor) |>
  rename(
    sci_eng_bachelors_percent = "Estimate!!Percent!!Total population 25 years and over with a Bachelor's degree or higher!!Science and Engineering",
    business_bachelors_percent = "Estimate!!Percent!!Total population 25 years and over with a Bachelor's degree or higher!!Business",
    education_bachelors_percent = "Estimate!!Percent!!Total population 25 years and over with a Bachelor's degree or higher!!Education",
    bachelors_percent = "Estimate!!Percent!!Total population 25 years and over with a Bachelor's degree or higher"
  ) |>
  select(stateabb, acs_district, bachelors_percent, sci_eng_bachelors_percent, business_bachelors_percent, education_bachelors_percent)

trim_acs_education <- trim_acs_dataset(acs_education) |>
  rename(
    less_than_hs_percent = "Estimate!!Percent!!AGE BY EDUCATIONAL ATTAINMENT!!Population 18 to 24 years!!Less than high school graduate",
    hs_graduate_percent = "Estimate!!Percent!!AGE BY EDUCATIONAL ATTAINMENT!!Population 18 to 24 years!!High school graduate (includes equivalency)",
    some_college_percent = "Estimate!!Percent!!AGE BY EDUCATIONAL ATTAINMENT!!Population 18 to 24 years!!Some college or associate's degree",
    higher_ed_percent = "Estimate!!Percent!!AGE BY EDUCATIONAL ATTAINMENT!!Population 18 to 24 years!!Bachelor's degree or higher"
  ) |>
  select(stateabb, acs_district, less_than_hs_percent, hs_graduate_percent, some_college_percent, higher_ed_percent)

trim_acs_enrollment <- trim_acs_dataset(acs_enrollment) |>
  rename(
    higher_ed_enrollment = "Estimate!!Percent!!Population enrolled in college or graduate school",
    higher_ed_18_24_enrollment = "Estimate!!Percent!!Population 18 to 24 years!!Enrolled in college or graduate school",
    k12_enrollment = "Estimate!!In public school!!Population 3 years and over enrolled in school!!Kindergarten to 12th grade"
  ) |>
  select(stateabb, acs_district, higher_ed_enrollment, higher_ed_18_24_enrollment, k12_enrollment)

trim_acs_marital <- trim_acs_dataset(acs_marital) |>
  rename(
    married_percent = "Estimate!!Now married (except separated)!!Population 15 years and over...97",
    never_married_percent = "Estimate!!Never married!!Population 15 years and over...353",
    divorced_percent = "Estimate!!Divorced!!Population 15 years and over...225",
    widowed_percent = "Estimate!!Widowed!!Population 15 years and over...131"
  ) |>
  select(stateabb, acs_district, married_percent, never_married_percent, divorced_percent, widowed_percent)
...
best_sim_bachelor <- get_best_similarities(test_mth_all_admindist_gys, trim_acs_bachelor, 'seda_district', 'acs_district', 0.3, 6)

best_sim_education <- get_best_similarities(test_mth_all_admindist_gys, trim_acs_education, 'seda_district', 'acs_district', 0.3, 6)

best_sim_enrollment <- get_best_similarities(test_mth_all_admindist_gys, trim_acs_enrollment, 'seda_district', 'acs_district', 0.3, 6)

best_sim_marital <- get_best_similarities(test_mth_all_admindist_gys, trim_acs_marital, 'seda_district', 'acs_district', 0.3, 6)

best_sim_csdh <- get_best_similarities(test_mth_all_admindist_gys, trim_csdh, 'seda_district', 'DistrictName', 0.3, 6)

best_sim_census_counties <- get_best_similarities(test_mth_all_admindist_gys, trim_census_sdc, 'seda_district', 'School District Name', 0.3, 3)

best_sim_scp_data <- get_best_similarities(test_mth_all_admindist_gys, trim_scp_county_data, 'seda_district', 'County', 0.9, 10)
