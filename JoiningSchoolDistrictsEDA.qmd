...
acs_child_characteristics <- readr::read_csv('data/ACS/acs-child-characteristics_2021.csv', skip = 1)
...
ccd_financial <- read_sas(unz("data/CCD/sdf22_1a_sas7bdat.zip", "sdf22_1a.sas7bdat"))


#ccd <- read_csv('data/CCD/ccd_lea_029_2324_w_0a_050824.csv')
...
```{r}
#| label: prepare-data-for-joining
...
trim_acs_housing <- trim_acs_dataset(acs_housing) |>
  ...
  select(stateabb, acs_district, occupancy_percentage)
```
...
get_best_similarities <- function(
  ...
  best_similarities <- similarities |>
    ...
    group_by(stateabb, !!sym(column1)) |>
    slice_min(order_by = dist, with_ties = TRUE) |>  # Find minimum dist, allowing ties
    slice_min(order_by = jaccard) |>                 # Break ties by selecting the minimum jaccard
    ungroup() |>
    arrange(jaccard)
...
best_sim_poverty <- get_best_similarities(test_mth_all_admindist_gys, trim_acs_poverty, 'seda_district', 'acs_district', 0.3, 3)

best_sim_housing <- get_best_similarities(test_mth_all_admindist_gys, trim_acs_housing, 'seda_district', 'acs_district', 0.3, 3)

best_sim_financial <- get_best_similarities(test_mth_all_admindist_gys, trim_ccd_financial, 'seda_district', 'ccd_district', 0.3, 3)
...
comprehensive_dataset <- test_mth_all_admindist_gys |>
  ...
  select(sedaadmin, seda_district.x, poverty_percent, occupancy_percentage, total_revenue, total_fed_revenue, gys_mn_2022_ol.x)
```
