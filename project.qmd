...
fipscodes <- readr::read_csv('data/all-geocodes-v2021.csv', skip = 4)

fipscodes_clean <- fipscodes |>
  rename(
...
fipsCoded <- merge(comp_mapping, fipscodes_clean, by = c("state_fips", "county_fips")) |>
  mutate(
...
county_map <- merge(fipsCoded, us_map, by = c("state", "subregion"))
...
plot_usmap(data = state_grouped_all, values = "mean_ol", regions = "state") +
